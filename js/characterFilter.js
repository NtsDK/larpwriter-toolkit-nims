...
CharacterFilter.refresh = function () {
    ...
    for (var i = 0; i < Database.ProfileSettings.length; ++i) {
        CharacterFilter.appendInput(filterSettingsDiv,
                Database.ProfileSettings[i]);
    }
                    ...
                    function (inputItemName) {
                        ...
                        case "checkbox":
                            ...
                            if (inputItem.options[0].selected) {
                                selectedOptions[inputItem.options[0].value === "Да" ? "true"
                                        : "false"] = true;
                            }
                            if (inputItem.options[1].selected) {
                                selectedOptions[inputItem.options[1].value === "Да" ? "true"
                                        : "false"] = true;
                            }
                        ...
                        case "number":
                            var num = new Number(
                                    filterSettingsDiv.inputItems[inputItem.selfInfo.name
                                            + ":numberInput"].value);
                        ...
                        case "string":
                            var regex = Utils
                                    .globStringToRegex(inputItem.value);
                            result = element[inputItem.selfInfo.name]
                                    .match(regex);
                            break;
                        }
    ...
    profileSettings.forEach(function (profileItemInfo) {
        ...
        if (profileItemInfo.type === "checkbox") {
            td.appendChild(document
                    .createTextNode(character[profileItemInfo.name] ? "Да"
                            : "Нет"));
        } else if (profileItemInfo.type === "text"
                && profileItemInfo.name !== "name") {
            var regex = Utils
                    .globStringToRegex(inputItems[profileItemInfo.name].value);
            var pos = character[profileItemInfo.name].search(regex);
            td.appendChild(document
                    .createTextNode(character[profileItemInfo.name].substring(
                            pos - 5, pos + 15)));
        } else {
            td.appendChild(document
                    .createTextNode(character[profileItemInfo.name]));
        }
        tr.appendChild(td);
    });

...
class L10nManager {
  ...
  checkDictionaryInsertCount(base, dictName) {
    ...
    const notEqual = intersection.filter(
      (key) => CU.strFormatInsertsCount(baseInst[key]) !== CU.strFormatInsertsCount(dictInst[key])
    );
  ...
  localizeStatic(el) {
    ...
    U.nl2array(U.qees(el, "[l10n-id]")).map((el2) =>
      U.addEl(U.clearEl(el2), U.makeText(this.getValue(U.getAttr(el2, "l10n-id"))))
    );
    U.nl2array(U.qees(el, "[l10n-placeholder-id]")).map((el2) =>
      U.setAttr(el2, "placeholder", this.getValue(U.getAttr(el2, "l10n-placeholder-id")))
    );
    U.nl2array(U.qees(el, "[l10n-title]")).map((el2) =>
      U.setAttr(el2, "title", this.getValue(U.getAttr(el2, "l10n-title")))
    );
  }

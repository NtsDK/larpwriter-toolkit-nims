...
module.exports = function (LocalDBMS, opts) {
  const { R, addListener, Errors, Constants, CU, PC } = opts;
  ...
  LocalDBMS.prototype.createPlayer = function ({ userName, password } = {}, user) {
    return new Promise((resolve, reject) => {
      ...
      const chain = [
        PC.createEntityCheck2(userName, R.keys(playersInfo), "entity-living-name", "entity-of-player"),
        PC.isString(password),
      ...
      PC.precondition(PC.chainCheck(chain), reject, () => {
        ...
        this.createProfile({ type: "player", characterName: userName }, user).then(() => {
          ...
          that.setPassword({ username: userName, type: "player", password }).then((res) => resolve(), reject);
        }, reject);
      });
    });
  };
  ...
  LocalDBMS.prototype.changePlayerPassword = function ({ userName, newPassword } = {}) {
    return new Promise((resolve, reject) => {
      const playersInfo = this.database.ManagementInfo.PlayersInfo;
      const chain = [
        PC.entityExistsCheck(userName, R.keys(playersInfo)),
        PC.isString(newPassword),
      ...
      PC.precondition(PC.chainCheck(chain), reject, () => {
        ...
        this.getUser({ username: userName, type: "player" }).then((user) => {
          ...
          that
            .setPassword({ username: userName, type: "player", password: newPassword })
            .then((res) => resolve())
            .catch(reject);
        }, reject);
      });
  ...
  LocalDBMS.prototype.createPlayerLogin = function ({ userName, password } = {}) {
    return new Promise((resolve, reject) => {
      ...
      const chain = [
        PC.entityExistsCheck(userName, R.keys(players)),
  ...
  LocalDBMS.prototype.removePlayerLogin = function ({ userName } = {}) {
    return new Promise((resolve, reject) => {
      PC.precondition(PC.entityExistsCheck(userName, R.keys(this.database.ManagementInfo.PlayersInfo)), reject, () => {

...
module.exports = function (LocalDBMS, opts) {
  const { R, addListener, Errors, Constants, CU, PC, serverSpecific } = opts;

  LocalDBMS.prototype.getManagementInfo = function () {
    return new Promise((resolve, reject) => {
      const { ManagementInfo } = this.database;
      const usersInfo = R.clone(
        R.keys(ManagementInfo.UsersInfo).reduce((result, user) => {
  ...
  LocalDBMS.prototype.createOrganizer = function ({ name, password } = {}) {
    return new Promise((resolve, reject) => {
      ...
      const chain = [
        PC.createEntityCheck2(name, R.keys(usersInfo), "entity-living-name", "entity-of-organizer"),
    ...
    return new Promise((resolve, reject) => {
      ...
      PC.precondition(PC.chainCheck(chain), reject, () => {
        ...
        this.getUser({ username: userName, type: "organizer" }).then((user) => {
          ...
          that
            .setPassword({ username: userName, type: "organizer", password: newPassword })
            .then((res) => resolve())
            .catch(reject);
        }, reject);
      });
  ...
  LocalDBMS.prototype.assignAdmin = function ({ name } = {}) {
    return new Promise((resolve, reject) => {
      PC.precondition(PC.entityExistsCheck(name, R.keys(this.database.ManagementInfo.UsersInfo)), reject, () => {

    ...
    LocalDBMS.prototype.log = function ({ userName, time, funcName, rewrite, params, status } = {}) {
      return new Promise((resolve, reject) => {
        const chain = PC.chainCheck([
          PC.isString(userName),
          PC.isString(time),
          PC.isString(funcName),
          PC.isBoolean(rewrite),
          PC.isArray(params),
          PC.isString(status),
        ]);
        PC.precondition(
    ...
    LocalDBMS.prototype.getLog = function ({ pageNumber, filter } = {}) {
      return new Promise((resolve, reject) => {
        ...
        PC.precondition(chain, reject, () => {
          const chain2 = PC.chainCheck(
            [PC.elementsFromEnum(R.keys(filter), Constants.logFilterTypes)].concat(R.values(filter).map(PC.isString))
          );
          PC.precondition(chain2, reject, () => {
            const tmp = this.database.Log.map((arr, i) => [i + 1].concat(arr)).filter((arr) =>
              filterMap.every((pair) => {

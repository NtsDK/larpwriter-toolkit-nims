...
((callback2) => {
  function storyCharactersAPI(LocalDBMS, opts) {
    const { R, Errors, addListener, Constants, CU, PC } = opts;
    ...
    LocalDBMS.prototype.addCharacterToEvent = function ({ storyName, eventIndex, characterName } = {}) {
      return new Promise((resolve, reject) => {
        ...
        PC.precondition(PC.chainCheck(chain), reject, () => {
          const story = this.database.Stories[storyName];
          chain = [
            PC.entityExistsCheck(characterName, R.keys(story.characters)),
    ...
    function _renameCharacterInStories([{ type, fromName, toName }] = []) {
      ...
      R.values(this.database.Stories)
        .filter((story) => story.characters[fromName] !== undefined)
        .forEach((story) => {

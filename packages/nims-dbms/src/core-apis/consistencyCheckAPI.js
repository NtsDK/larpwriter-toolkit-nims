...
((callback2) => {
  function consistencyCheckAPI(LocalDBMS, opts) {
    const { R, CU, Constants, dbmsUtils } = opts;
    ...
    function checkBindingsConsistency(data) {
      const errors = [];
      R.toPairs(R.invert(data.ProfileBindings))
    ...
    function checkRelationsConsistency(data, callback) {
      ...
      R.values(groups)
        .filter(R.pipe(R.length, R.gt(R.__, 1)))
        .forEach((group) => {
    ...
    function checkStoryCharactersConsistency(data, callback) {
      ...
      R.values(data.Stories).forEach((story) => {
        ...
        if (differenceOuter.length !== 0) {
          const msg =
            "Story characters inconsistent, inner and outer character name are inconsistent: story {0}, character {1}";

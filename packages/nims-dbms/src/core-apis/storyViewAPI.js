...
((callback2) => {
  function storyViewAPI(LocalDBMS, opts) {
      ...
      return new Promise((resolve, reject) => {
        PC.precondition(characterCheck(characterName, this.database), reject, () => {
          const array = R.values(this.database.Stories)
            .filter(
              (story) =>
                story.characters[characterName] !== undefined && story.characters[characterName].inventory !== ""
            )
            .map((story) => ({
      ...
      return new Promise((resolve, reject) => {
        PC.precondition(characterCheck(characterName, this.database), reject, () => {
          ...
          Object.keys(this.database.Stories)
            .filter((storyName) => that.database.Stories[storyName].characters[characterName])
            .forEach((storyName) => {
              events = [];
    ...
    // character filter
    LocalDBMS.prototype.getCharactersSummary = function () {
        ...
        PC.precondition(characterCheck(characterName, this.database), reject, () => {
          const characterReport = R.values(this.database.Stories)
            ...
            .map((story) => {
              ...
              const finishedAdaptations = charEvents.filter(
                (event) => event.characters[characterName].ready === true
              ).length;

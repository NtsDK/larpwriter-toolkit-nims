...
((callback2) => {
  function groupSchemaAPI(LocalDBMS, opts) {
      ...
      return superKeys.every((superKey) => {
        ...
        switch (superItem.type) {
          ...
          case "multiEnum":
            ...
            if (subItem.condition === "equal" && superItem.condition === "equal") {
              return (
                R.symmetricDifference(R.keys(superItem.selectedOptions), R.keys(subItem.selectedOptions)).length === 0
              );
            }
    ...
    const _makeGraph = (equalGroups, superGroups, groupCharacterSets) => {
      ...
      const edges = R.keys(superGroups).reduce(
        (result, subGroup) =>
          result.concat(
            R.keys(superGroups[subGroup]).map((superGroup) => ({
              from: subGroup,
              to: superGroup,
              arrow: "to",
            }))
    ...
    const _makeGroupSchema = (groups, _isGroupsEqual, _isSuperGroup, _extractKeyInfo, groupCharacterSets) => {
      ...
      const uniqueGroups = R.toPairs(groupNamesSet)
        .filter((item) => item[1])
        .map(R.head);
    ...
    LocalDBMS.prototype.getGroupSchemas = function () {
      return new Promise((resolve, reject) => {
        const that = this;
        this.getGroupCharacterSets()
          .then((groupCharacterSets) => {
            const schemas = {};
            const groups = that.database.Groups;

            schemas.theory = _makeGroupSchema(
              groups,
              _isGroupsEqualByFilterModel,
              _isSuperGroupByFilterModel,
              (groupName) => groups[groupName].filterModel,
              groupCharacterSets
            );

            schemas.practice = _makeGroupSchema(
              groups,
              _isGroupsEqualByElements,
              _isSuperGroupByElements,
              (groupName) => groupCharacterSets[groupName],
              groupCharacterSets
            );

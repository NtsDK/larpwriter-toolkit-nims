...
((callback2) => {
  function briefingExportAPI(LocalDBMS, opts) {
    const { R, Constants, dbmsUtils, CU, PC } = opts;
    ...
    let _getBriefingData,
      _makeProfileInfo,
      _makeRelationsInfo,
      _makeCharInventory,
      _getProfileInfoNotEmpty,
      _getSimpleProfileInfoObject,
      _getSplittedProfileInfoObject,
      _getProfileInfoArray,
      _getStoriesInfo,
      _getEventsInfo,
      _getStoryEventsInfo,
      _makeEventInfo,
    ...
    // DBMS.briefings.get()
    LocalDBMS.prototype.getBriefingData = function ({ selCharacters, selStories, exportOnlyFinishedStories } = {}) {
    ...
    _makeCharInventory = (database, charName) =>
      R.values(database.Stories)
        ...
        .map((story) => story.characters[charName].inventory)
        .join(", ");
    ...
    _getStoryEventsInfo = (story, charName, defaultTime) =>
      story.events.filter((event) => event.characters[charName]).map(_makeEventInfo(charName, story.name, defaultTime));

...
((callback2) => {
  function textSearchAPI(LocalDBMS, opts) {
    ...
    searchers.writerStory = (textType, test, database) =>
      R.values(database.Stories)
        .filter((story) => test(story.story))
        .map((story) => format(story.name, "text", story.story));

    searchers.eventOrigins = (textType, test, database) =>
      R.flatten(
        R.values(database.Stories).map((story) =>
          story.events
    ...
    searchers.eventAdaptations = (textType, test, database) =>
      R.flatten(
        R.values(database.Stories).map((story) =>
          story.events.map((event) =>
            R.keys(event.characters)
              .filter((char) => test(event.characters[char].text))
              .map((char) => format(`${story.name}/${event.name}/${char}`, "text", event.characters[char].text))
          )
        )
      );
    ...
    const profileSearch = R.curry((profiles, structure, textType, test, database) => {
      ...
      return R.flatten(
        R.values(database[profiles]).map((profile) =>
          items
            .filter((item) => test(profile[item.name]))

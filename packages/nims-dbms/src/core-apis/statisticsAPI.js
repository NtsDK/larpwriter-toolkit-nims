  ...
  function statisticsAPI(LocalDBMS, opts) {
    ...
    LocalDBMS.prototype.getStatistics = function () {
      return new Promise((resolve, reject) => {
        const that = this;
        this.getAllCharacterGroupTexts()
          .then((groupTexts) => {
    ...
    function _getProfileChartArray(database, profileType, profileStructureType) {
      ...
      const groupReduce = (group) =>
        R.fromPairs(
          R.toPairs(group).map((elem) => {
    ...
    function _getStoryCompleteness(database) {
      ...
      const finishedStories = R.values(database.Stories)
        ...
        .filter((stats) => stats.allAdaptations === stats.finishedAdaptations && stats.allAdaptations !== 0).length;
      return {
    ...
    function _countTextCharacters(database) {
      ...
      counts.groups = R.sum(
        R.values(database.Groups).map(R.compose(_noWhiteSpaceLength, R.prop("characterDescription")))
      );

...
((callback2) => {
  function groupsAPI(LocalDBMS, opts) {
    const { R, Constants, Errors, addListener, CU, PC, ProjectUtils: PU } = opts;
    ...
    LocalDBMS.prototype.getCharacterGroupTexts = function ({ characterName } = {}) {
      return new Promise((resolve, reject) => {
        Promise.all([this.getProfileBinding({ type: "character", name: characterName }), this.getProfileFilterInfo()])
          .then((results) => {
    ...
    LocalDBMS.prototype.getAllCharacterGroupTexts = function () {
      return new Promise((resolve, reject) => {
        const that = this;
        Promise.all([this.getProfileFilterInfo(), this.getProfileBindings()])
          .then((results) => {
      ...
      return new Promise((resolve, reject) => {
        PC.precondition(groupCheck(groupName, this.database), reject, () => {
          ...
          if (conflictTypes.length !== 0) {
            reject(
              new Errors.ValidationError("groups-page-filter-is-incompatible-with-base-profiles", [
                conflictTypes.join(","),
              ])
            );
            return;

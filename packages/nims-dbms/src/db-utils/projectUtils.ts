...
export const makeGroupedProfileFilterInfo = (opts) => {
  ...
  arr = arr.concat(
    opts.characters.profileStructure.map((element) => ({
  ...
  arr = arr.concat(
    opts.players.profileStructure.map((element) => ({
...
const getCharacterInfoValue2 = (info, profileId, profileItemName) => {
  if (
    profileItemName === Constants.CHAR_NAME ||
    ...
    profileItemName === Constants.PLAYER_NAME ||
...
export const getDataArray = R.curry((info, profileId) =>
  R.flatten(info.groupedProfileFilterItems.map(R.prop("profileFilterItems"))).map((profileItemInfo) => {
...
export const getDataArrays = (info, filterModel) =>
  info.bindingData.map(getDataArray(info)).filter(acceptDataRow(filterModel));

...
const behaviors = {
  ...
  open: {
    accept: ["organizer", "editor", "admin", "adminEditor", "player", "noUser"],
    reject: [],
  },
};
...
const testProfile = R.curry(
  (permissionProxy, db, check, okExpected, profileName) =>
...
exports.test = function (permissionProxy, db) {
  ...
  Promise.all(
    R.values(profiles)
      .filter(R.has("beforeAll"))
      .map(
        (profile) => db[profile.beforeAll[0]](profile.beforeAll[1], { name: "admin" })
    ...
    .then(() => {
      ...
      Promise.all(
        R.values(profiles)
          .filter(R.has("afterAll"))
          .map((profile) => db[profile.afterAll[0]](profile.afterAll[1], { name: "admin" }))
      ).catch((err) => {
        console.error(err);
        process.exit(1);
      });

...
const userIsLogged = (args, user, db) =>
  new Promise((resolve, reject) => {
    // eslint-disable-next-line no-unused-expressions
    user ? resolve() : reject(["errors-user-is-not-logged"]);
  });
const roleIsOrganizer = (args, user, db) =>
  new Promise((resolve, reject) => {
    // eslint-disable-next-line no-unused-expressions
    user.role === "organizer" ? resolve() : reject(["errors-forbidden-for-role", [user.role]]);
  });
const organizerIsAdmin = (args, user, db) =>
  new Promise((resolve, reject) => {
    db.getManagementInfo()
      .then((res) => (res.admin === user.name ? resolve() : reject(["errors-forbidden-for-non-admin"])))
      .catch(reject);
  });
const organizerIsEditor = (args, user, db) =>
  new Promise((resolve, reject) => {
    db.getManagementInfo()
      .then((res) => (res.editor === user.name ? resolve() : reject(["errors-forbidden-for-non-editor"])))
      .catch(reject);
  });
const canPlayerCreateChar = (args, user, db) =>
  new Promise((resolve, reject) => {
    db.getPlayersOptions()
      .then((res) => (res.allowCharacterCreation ? resolve() : reject(["errors-forbidden-to-create-char"])))
      .catch(reject);
  });
const checkEditorMode = (args, user, db) =>
  new Promise((resolve, reject) => {
    db.getManagementInfo()
      .then((res) => {
        if (res.editor === null) {
...
const isProfileOwner = R.curry(
  (property, args, user, db) =>
    new Promise((resolve, reject) => {
      let { type } = args;
...
const isCharacterOwner = R.curry(
  (property, args, user, db) =>
    new Promise((resolve, reject) => {
      const story = args[property];
      db.getManagementInfo()
...
const isAdaptationOwner = (args, user, db) =>
  new Promise((resolve, reject) => {
    db.getManagementInfo()
      .then((res) => {
...
const isPlayerProfileOwner = (args, user, db) =>
  new Promise((resolve, reject) => {
...
const canUpdateProfileField = (property, args, user, db) =>
  new Promise((resolve, reject) => {
...
const roleIsOrganizerCheck = (args, user, db) =>
  Promise.all([userIsLogged(args, user, db), roleIsOrganizer(args, user, db)]);
const organizerIsAdminCheck = (args, user, db) =>
  Promise.all([roleIsOrganizerCheck(args, user, db), organizerIsAdmin(args, user, db)]);
const organizerIsAdminOrEditorCheck = (args, user, db) =>
  Promise.all([
    roleIsOrganizerCheck(args, user, db),
    or(organizerIsAdmin(args, user, db), organizerIsEditor(args, user, db)),
  ]);
const canPlayerCreateCharCheck = (args, user, db) =>
  Promise.all([userIsLogged(args, user, db), canPlayerCreateChar(args, user, db)]);
const isProfileOwnerCheck = R.curry((property, args, user, db) =>
  Promise.all([
    roleIsOrganizerCheck(args, user, db),
...
const isGroupOwnerCheck = R.curry((property, args, user, db) =>
  Promise.all([
    roleIsOrganizerCheck(args, user, db),
    checkEditorMode(args, user, db),
...
const isStoryOwnerCheck = R.curry((property, args, user, db) =>
  Promise.all([
    roleIsOrganizerCheck(args, user, db),
    checkEditorMode(args, user, db),
...
const isAdaptationOwnerCheck = R.curry((args, user, db) =>
  Promise.all([
    roleIsOrganizerCheck(args, user, db),
    checkEditorMode(args, user, db),
...
exports.applyPermissionProxy = R.curry(
  (makeValidationError, dbms) =>
    new Proxy(dbms, {
      get(target, prop) {

  ...
  function refresh() {
    Promise.all([
      ...
      .then((results) => {
        ...
        const groupFilterData = group.filterModel.map((el) =>
          makeFilterItemString(t, name2DisplayName, name2Source, el)
        );
                  ...
                  <tbody>
                    {groupFilterData.map((rowData) => (
                      <tr className="GroupFilterRow">
                        <td className="profile-item" title={rowData.title}>
                          {rowData.displayName}
                        </td>
  ...
  switch (filterItem.type) {
    ...
    case "checkbox":
      ...
      if (filterItem.selectedOptions.true) {
        arr.push(t("constant-yes"));
      }
      if (filterItem.selectedOptions.false) {
        arr.push(t("constant-no"));
      }
      condition = t("groups.one-from");
      value = arr.join(", ");
      break;

          ...
          groupNames.map((name, i, arr) => {
            ...
            if (name.editable) {
              ...
              U.listen(
                removeBtn,
                "click",
                removeGroup(() => name.value, this.refresh, removeBtn)
              );
            } else {
        ...
        Object.keys(inputItems).forEach((inputName) => {
          if (inputItems[inputName].type === "checkbox") {
            ...
            if (inputName === "filterModel") {
              ...
              U.addEls(inputItem, [
                U.makeText(data.join(", ")),
                U.makeEl("br"),
                U.makeText(L10n.getValue("groups-total") + data.length),
              ]);
            } else {
    ...
    switch (filterItem.type) {
      ...
      case "checkbox":
        ...
        if (filterItem.selectedOptions.true) {
          arr.push(L10n.getValue("constant-yes"));
        }
        if (filterItem.selectedOptions.false) {
          arr.push(L10n.getValue("constant-no"));
        }
        condition = L10n.getValue("groups-one-from");
        value = arr.join(", ");
        break;
...
export const removeGroup = (callback, refresh, btn) => () => {
  ...
  UI.confirm(CU.strFormat(L10n.getValue("groups-are-you-sure-about-group-removing"), [name]), () => {
    DBMS.removeGroup({ groupName: name })
      .then(() => {
        PermissionInformer.refresh()
          .then(() => {

...
export class ProfileFilter {
  ...
  init() {
    ...
    U.listen(
      U.queryEl(`${root}#profile-filter-columns .profile-item-selector`),
      "change",
      UI.showSelectedEls3(root, "dependent", "dependent-index")
    );

    //        UI.enable(content, 'isGroupEditable', isGroupEditable);
    //        listen U.queryEl(`${root}.save-entity-select`)

    $(`${root}.save-entity-select`)
      .select2()
      .on("change", (event) => {
    ...
    U.listen(
      U.queryEl(`${root}.remove.group`),
      "click",
      removeGroup(() => U.queryEl(`${root}.save-entity-select`).value, this.refresh)
    );
    ...
    FilterConfiguration.makeFilterConfiguration(DBMS)
      .then((filterConfiguration) => {
        ...
        U.addEls(
          filterSettingsDiv,
          R.flatten(
            groupedProfileFilterItems.map((item) =>
              R.concat(item.profileFilterItems.map(this.makeInput), [U.addClass(U.makeEl("div"), "filterSeparator")])
            )
          )
        );
  ...
  onAddFilterCondition() {
    this.state.addFilterConditionDialog.showDlg(
      this.state.filterConfiguration.getGroupsForSelect(),
      this.setFilterModelItem
    );
  }
  ...
  makePrintData() {
    ...
    const sortFunc = CU.charOrdAFactoryBase(
      this.state.sortDir,
      (a, b) => a > b,
      (a) => {
    ...
    DBMS.getGroup({ groupName })
      .then((group) => {
        ...
        if (conflictTypes.length !== 0) {
          UI.alert(
            CU.strFormat(L10n.getValue("groups-base-filter-is-incompatible-with-page-profiles"), [
              conflictTypes.join(","),
            ])
          );
          return;
  ...
  downloadFilterTable() {
    ...
    const dataArrays = this.makePrintData();
    const cleanArrays = dataArrays.map((dataArray) =>
      dataArray.filter((item, index) => selected[index]).map(R.prop("value"))
    );
  ...
  makeDataString(dataArray) {
    ...
    return U.addEls(
      ...
      dataArray.map((valueInfo, i) => {
        ...
        const td = U.addEl(
          U.setClassByCondition(U.makeEl("td"), "lightGrey", value === undefined),
          U.makeText(displayValue)
        );
        U.addClasses(td, [
          `dependent-${i}`,
          "dependent",
          valueInfo.type === "number" ? "text-align-right" : "text-align-left",
        ]);
    ...
    return U.addEls(
      ...
      profileItemNames.map((elem, i) => {
        ...
        U.addClasses(td, [
          ...
          "dependent",
          "sorting",
          elem.type === "number" ? "text-align-right" : "text-align-left",
        ]);
        U.setAttr(td, "dependent-index", i);
  ...
  renameGroup(selector) {
    return (dialog) => () => {
      ...
      DBMS.renameGroup({ fromName, toName })
        .then(() => {

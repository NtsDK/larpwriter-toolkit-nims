...
export function EntityNav(props) {
  const { entityList, createEntityText, entityUrl, createEntityDialog, getEntityDropdown } = props;
    ...
    <div className="EntityNav panel panel-default  entity-select height-100p">
      <div className="panel-body height-100p">
        <div className="flex-row entity-toolbar">
          ...
          <ModalTrigger modal={createEntityDialog}>
            <Button className="btn-reduced fa-icon create flex-0-0-auto" title={t(createEntityText)} />
          </ModalTrigger>
        </div>
        <ul className="entity-list ">
          {entityList.map((entity) => (
            <li className="flex-row">
              <NavLink
                className="btn btn-default flex-1-1-auto text-align-left"
                to={`${entityUrl}/${entity.primaryName}`}
              >

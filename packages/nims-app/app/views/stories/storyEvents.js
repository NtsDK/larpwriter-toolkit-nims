...
import { getStoryEventsTemplate, getCreateEventBody, getMoveEventBody } from "./StoryEventsTemplate.jsx";
...
export default function createStoryEvents(Stories) {
  ...
  function rebuildInterface(events, metaInfo) {
    ...
    R.ap(
      [U.addEl(table)],
      events.map((event, i, events2) => appendEventInput(event, i, events2, metaInfo.date, metaInfo.preGameDate))
    );

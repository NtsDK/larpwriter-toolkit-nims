...
export default function createStoryCharacters(Stories) {
  ...
  function init() {
    ...
    state.ExternalCharacterSelectors = [
      U.queryEl(`${superRoot}.storyCharactersAddSelector`),
  ...
  function refresh() {
    ...
    if (!Stories.getCurrentStoryName()) {
      return;
    }
  ...
  function rebuildInterface(allCharacters, localCharacters) {
    ...
    allCharacters
      .filter((nameInfo) => !localCharacters[nameInfo.value])
      .forEach((nameInfo) => {

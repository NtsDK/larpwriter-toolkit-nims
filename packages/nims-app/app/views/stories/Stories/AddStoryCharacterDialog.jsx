...
export function AddStoryCharacterDialog(props) {
  const { storyName, onAdd, ...elementProps } = props;
  ...
  function refresh() {
    ...
    Promise.all([
      ...
      .then((results) => {
        ...
        const charGroups = R.groupBy(
          (nameInfo) => (localCharacters[nameInfo.value] ? "inStoryChars" : "outOfStoryChars"),
          allCharacters
        );
        setState({
          ...charGroups,

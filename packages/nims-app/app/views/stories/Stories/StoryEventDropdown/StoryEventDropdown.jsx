...
export function StoryEventDropdown(props) {
  const { event, nextEvent, storyName, refresh } = props;
  ...
  return (
    <Dropdown pullRight>
      <Dropdown.Toggle noCaret className="btn btn-default fa-icon kebab story-event-dropdown-btn" />
      <Dropdown.Menu>
        <MenuItem onClick={cloneEvent}>{t("stories.clone-event")}</MenuItem>
        {!!nextEvent && (
          <ModalTrigger
            modal={
              <MergeStoryEventsDialog storyName={storyName} event={event} nextEvent={nextEvent} onMerge={refresh} />
            }
          >
            <MenuItem>{t("stories.merge-events")}</MenuItem>
          </ModalTrigger>
        )}
        <MenuItem divider />
        <ModalTrigger modal={<RemoveStoryEventDialog storyName={storyName} event={event} onRemove={refresh} />}>
          <MenuItem>{t("stories.remove-event")}</MenuItem>
        </ModalTrigger>
      </Dropdown.Menu>
    </Dropdown>
  );

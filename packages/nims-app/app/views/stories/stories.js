...
Stories.chainRefresh = () => {
  if (state.left.сurrentViewNameIs("EventPresence") || state.right.сurrentViewNameIs("EventPresence")) {
...
function renameStory(dialog) {
  return () => {
    ...
    DBMS.renameStory({ fromName, toName })
      .then(() => {
...
function removeStory() {
  ...
  UI.confirm(CU.strFormat(L10n.getValue("stories-are-you-sure-about-story-removing"), [name]), () => {
    DBMS.removeStory({ storyName: name })
      .then(() => {
        PermissionInformer.refresh()
          .then(() => {
            Stories.refresh();
...
function onStorySelectorChange(storyName) {
  ...
  if (storyName) {
    ...
    // when there are no stories at all
    updateSettings(null);
    state.left.refreshCurrentView();
    state.right.refreshCurrentView();
  }

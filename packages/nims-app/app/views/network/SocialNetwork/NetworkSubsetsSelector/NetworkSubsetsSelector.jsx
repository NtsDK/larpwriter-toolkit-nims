...
export class NetworkSubsetsSelector extends Component {
  ...
  onTypeChange(e) {
    const { onSubsetChange, subset, characterNames, storyNames } = this.props;
  ...
  getDrawCharacterNames(type, selectedCharacterNames, selectedStoryNames) {
    const { characterNames, Stories } = this.props;
    // const { value } = U.queryEl('#networkSubsetsSelector');

    if (Constants.objectSubsets[0] === type) {
      // all objects
      return R.pluck("value", characterNames);
    }
    if (Constants.objectSubsets[1] === type) {
      // "selected characters"
      ...
      const secondaryCharacters = R.values(Stories)
        ...
        .map((story) =>
          story.events
            .filter((event) => R.keys(event.characters).some(isPrimaryCharacter))
    ...
    if (Constants.objectSubsets[2] === type) {
      //"selected stories"
  ...
  getDrawStoryNames(type, selectedCharacterNames, selectedStoryNames) {
    const { storyNames, Stories } = this.props;
  ...
  render() {
    ...
    const { t, subset, characterNames, storyNames } = this.props;
    ...
    return (
      <div className="NetworkSubsetsSelector network-filter-area">
        ...
        >
          {Constants.objectSubsets.map((el) => (
            <option value={el}>{t(`constant.${el}`)}</option>
          ))}
        </select>

  ...
  const info = colorGroups.reduce(
    (acc, group) => {
      if (group.type === "enum") {
        ...
        acc.groupColors[`${PROFILE_GROUP + group.name}.${trueName}`] =
          Constants.colorPalette[group.value ? 0 + 2 : 1 + 2];
        acc.groupColors[`${PROFILE_GROUP + group.name}.${falseName}`] =
          Constants.colorPalette[group.value ? 1 + 2 : 0 + 2];
        acc.groupLists[PROFILE_GROUP + group.name] = [
...
export class NodesColoringSelector extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (profileStructure !== prevProps.profileStructure || groupCharacterSets !== prevProps.groupCharacterSets) {
      const networkNodeGroups = this.getNetworkNodeGroups();
      // const info = getGroupColors(profileStructure);
      this.setState({
        networkNodeGroups,
  ...
  getLegendList() {
    const { nodesColoring, t, networkSettings, groupColorsInfo } = this.props;
    ...
    if (selectedGroup === "noGroup") {
      ...
      els.push(
        ...groupLists[selectedGroup].map((value) => ({

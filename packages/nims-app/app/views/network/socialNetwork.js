...
function init() {
  ...
  U.queryEls("#activityBlock button").forEach(
    U.listen(R.__, "click", (event) => U.toggleClass(event.target, "btn-primary"))
  );
  U.queryEls("#relationsBlock button").forEach(
    U.listen(R.__, "click", (event) => U.toggleClass(event.target, "btn-primary"))
  );
...
function refresh() {
  ...
  Promise.all([
    ...
    .then((results) => {
      ...
      const profileGroups = colorGroups
        .map((group) => group.name)
        .map((name) => ({ value: PROFILE_GROUP + name, name: profileLabel([name]) }));
      const filterGroups = R.keys(groupCharacterSets).map((name) => ({
        value: FILTER_GROUP + name,
        name: filterLabel([name]),
      }));
      U.fillSelector(selector, [defaultColorGroup].concat(profileGroups).concat(filterGroups));
  ...
  colorGroups.forEach((group) => {
    if (group.type === "enum") {
      ...
      state.groupLists[PROFILE_GROUP + group.name] = [
        `${PROFILE_GROUP + group.name}.${trueName}`,
        `${PROFILE_GROUP + group.name}.${falseName}`,
      ];
    } else {
...
function refreshLegend(groupName) {
  ...
  if (groupName === "noGroup") {
    ...
    els = els.concat(
      state.groupLists[groupName].map((value) =>
...
function onNetworkSelectorChange(selectedNetwork) {
  ...
  switch (selectedNetwork) {
    ...
    case "characterPresenceInStory":
      nodes = getCharacterNodes(data.Characters, groupName, showPlayer, characterNames).concat(
        getStoryNodes(data.Stories, storyNames)
      );

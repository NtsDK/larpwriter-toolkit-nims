...
import { getAdaptationRow } from "../adaptations/AdaptationsTemplate.jsx";
  ...
  state.panels = [
    {
      name: "storyRights",
      load(data, callback) {
        PermissionInformer.getEntityNamesArray({ type: "story", editableOnly: true })
          .then((userStoryNames) => {
    ...
    },
    {
      ...
      load(data, callback) {
        DBMS.getProfile({ type: "character", name: data.characterName })
          .then((profile) => {
      ...
      make(el, data) {
        ...
        U.addEl(
          ...
          makePanel(
            U.makeText(label),
            makeProfileTable(Constants, state.characterProfileStructure, data.profile),
    ...
    },
    {
      name: "playerProfile",
      load(data, callback) {
        DBMS.getProfileBinding({ type: "character", name: data.characterName })
          .then((binding) => {
            if (binding[1] === "") {
              callback();
            } else {
      ...
      make(el, data) {
        if (data.playerProfile) {
          ...
          U.addEl(
            ...
            makePanel(
              U.makeText(label),
              makeProfileTable(Constants, state.playerProfileStructure, data.playerProfile),
              getFlags().hideAllPanels
            )
          );
        }
    ...
    },
    {
      name: "inventory",
      load(data, callback) {
        DBMS.getAllInventoryLists({ characterName: data.characterName })
          .then((allInventoryLists) => {
            data.allInventoryLists = allInventoryLists.sort(
              CU.charOrdAFactory(R.compose(R.toLower, R.prop("storyName")))
            );
            callback();
          })
          .catch(UI.handleError);
    ...
    },
    {
      name: "groups",
      load(data, callback) {
        DBMS.getCharacterGroupTexts({ characterName: data.characterName })
          .then((groupTexts) => {
    ...
    },
    {
      ...
      make(el, data) {
        ...
        const content = RelationsPreview.makeRelationsContent(
          data,
          getFlags().isAdaptationsMode,
          state.characterProfileStructure,
          refresh
        );
...
function showEventsByTime(content, characterName, userStoryNamesMap, flags) {
  DBMS.getCharacterEventsByTime({ characterName })
    .then((allEvents) => {
      ...
      Promise.all([PermissionInformer.areAdaptationsEditable({ adaptations }), DBMS.getMetaInfo()])
        .then((results) => {
          ...
          U.addEls(
            content,
            R.splitEvery(splitConstant, allEvents).map((subPart, i) => {
              const eventContent = U.addEls(
                U.makeEl("div"),
                subPart.map((event, j) => {
              ...
              if (flags.disableHeaders) {
                name = U.makeText(
                  CU.strFormat(L10n.getValue("briefings-events-header"), [
                    i * splitConstant + 1,
                    i * splitConstant + subPart.length,
                  ])
                );
              } else {
                name = U.addEls(
                  U.makeEl("div"),
                  subPart.map((event) => getEventHeaderDiv(event, true))
                );
              }
...
function showEventsByStory(content, characterName, userStoryNamesMap, flags) {
  DBMS.getCharacterEventGroupsByStory({ characterName })
    .then((eventGroups) => {
      ...
      Promise.all([PermissionInformer.areAdaptationsEditable({ adaptations }), DBMS.getMetaInfo()])
        .then((results) => {
          ...
          U.addEls(
            content,
            eventGroups.map((elem, i) => {
              const storyContent = U.addEls(
                U.makeEl("div"),
                elem.events.map((event, j) => {
                  opts.index = j + 1;
                  return showEvent(event, characterName, opts, flags);
                })
              );
...
function getEventHeaderDiv(event, showStoryName) {
  const eventName = U.addEl(
    U.makeEl("span"),
    U.makeText(CU.strFormat("{0} {1}", [showStoryName ? `${event.storyName}:` : "", event.name]))
  );
...
function showEvent(event, characterName, opts, flags) {
  ...
  const adaptationsCard = Adaptations.makeAdaptationCard(isEditable, event, event.storyName, characterName, {
    cardTitle: "",
    showTimeInput: showAll || showSubjectiveTime,
...
function updateCharacterInventory(event) {
  const { storyName, characterName, value } = event.target;

import { getTemplate } from "nims-resources";
...
function init() {
  ...
  U.listen(U.queryEl("#makeMarkdownBriefings"), "click", () => {
    makeTextBriefings(
      "html",
      R.compose((data) => markdownit("commonmark").render(data), generateSingleTxt(U.queryEl("#templateArea").value))
    );
  });
...
function refreshSetSelect(entityType, selectorName) {
  ...
  PermissionInformer.getEntityNamesArray({ type: entityType, editableOnly: false })
    .then((names) => {
      if (names.length > 0) {
...
function exportDocxByTemplate(template) {
  getBriefingData((err, briefingData) => {
    ...
    generateBriefings(
      briefingData,
      "docx",
      generateSingleDocx("blob", template),
      generateSingleDocx("Uint8Array", template)
    );
  });
}
...
function makeDocxTemplate(type) {
  ...
  const replaceBrackets = R.pipe(
    R.replace(/{{{/g, "{"),
    R.replace(/}}}/g, "}"),
    R.replace(/{{/g, "{"),
    R.replace(/}}/g, "}")
  );
  template = replaceBrackets(template)
    .split("\n")
    .map((string) => ({ string }));

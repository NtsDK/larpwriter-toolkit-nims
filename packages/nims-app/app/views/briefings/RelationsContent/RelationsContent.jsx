...
export class RelationsContent extends Component {
  ...
  refresh() {
    const { characterName, characterProfileStructure, dbms, permissionInformer } = this.props;
  ...
  onAddCharacterRelation(e) {
    const { characterName, dbms } = this.props;
    ...
    dbms
      .createCharacterRelation({ fromCharacter, toCharacter })
      .then(() => {
        this.refresh();
      })
      .catch(UI.handleError);
  ...
  render() {
    ...
    const { t, characterName, isAdaptationsMode, characterProfileStructure } = this.props;

...
function makeRelationsContent(data, isAdaptationsMode, profileSettings, externalRefresh) {
  const { characterName, relationsSummary, profiles, profileBindings } = data;
  ...
  const makeRow = makeNewRow(
    profiles,
    getProfileItemSelect,
    isAdaptationsMode,
    relationsSummary.knownCharacters,
    profileBindings,
    externalRefresh,
    characterName
  );
  ...
  // filling table
  const toCharacterFilter = (toCharacter) =>
    isAdaptationsMode
      ? true
  ...
  U.addEls(
    content,
    showCharacters.filter(toCharacterFilter).map((toChar) => makeRow(toChar, findRelTmp(toChar)))
  );
  return relationTmpl;
}
...
makeNewRow = R.curry(
  (
    ...
    U.listen(tmplQe("button.remove"), "click", (event) => {
      UI.confirm(
        CU.strFormat(l10n("are-you-sure-about-relation-removing"), [`${`${fromCharacter}-${toCharacter}`}`]),
        () => {
    ...
    fillFinishedButton(
      tmplQe(".direct .finished"),
      JSON.stringify([fromCharacter, toCharacter]),
      fromCharacter,
      toCharacter,
      fromCharacter,
      directChecked,
    ...
    fillFinishedButton(
      tmplQe(".reverse .finished"),
      JSON.stringify([toCharacter, fromCharacter]),
      fromCharacter,
      toCharacter,
      toCharacter,
      reverseChecked,
...
function fillCharSelector(select1, button, data, fromCharacter, makeRowCallback) {
  ...
  U.listen(button, "click", () => {
    ...
    DBMS.createCharacterRelation({ fromCharacter, toCharacter })
      .then(() => {
        DBMS.getCharacterRelation({ fromCharacter, toCharacter })
          .then((rel) => {

...
export class ProfileConstructor extends Component {
  ...
  refresh() {
    ...
    Promise.all([dbms.getProfileStructure({ type: "character" })])
      .then((results) => {
        ...
        <div className="panel panel-default max-height-100p overflow-auto">
          <div className="panel-body profile-panel">
            <div className="entity-management">
              <div>
                <ModalTrigger
                  modal={<CreateProfileItemDialog profileStructure={profileStructure} onCreate={this.refresh} />}
                >
                  <Button className="fa-icon create adminOnly icon-padding">{t("profiles.create-profile-item")}</Button>
                </ModalTrigger>
              </div>
            </div>
                ...
                {(provided) => (
                  <div className="tw-m-auto tw-max-w-screen-md" ref={provided.innerRef} {...provided.droppableProps}>
                    {profileStructure.map((profileStructureItem, i) => (
                      <ProfileConstructorRow2
                        key={profileStructureItem.name}
                        profileStructureItem={profileStructureItem}
                        i={i}

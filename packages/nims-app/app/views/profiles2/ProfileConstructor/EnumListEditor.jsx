...
export function EnumListEditor(props) {
  ...
  const { profileItemName, list, setList, setErrorText, canRemoveLast } = props;
    ...
    <div className="tw-flex tw-flex-wrap">
      {R.sort(CU.charOrdA, list).map((name) => {
        ...
        return (
          ...
          >
            <MenuItem onClick={renameEnumItem} data-enum-item-name={name}>
              {t("common.rename")}
            </MenuItem>
            {(canRemoveLast || list.length > 1) && (
              <>
                <MenuItem divider />
                <MenuItem onClick={removeEnumItem} data-enum-item-name={name}>
                  {t("common.remove")}
                </MenuItem>
              </>
            )}

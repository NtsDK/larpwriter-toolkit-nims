...
export function CreateProfileItemDialog(props) {
  ...
  const getPositionOptions = R.pipe(
    R.pluck("name"),
    R.map((name) => t("common.set-item-before2", { name }))
  );
  ...
  async function onSubmit({ itemName, itemPosition, itemType }) {
    return dbms
      .createProfileItem({
        ...
        name: itemName,
        itemType,
        selectedIndex: Number(itemPosition),
      })
      .then(onCreate)
    ...
    >
      ...
      <FormGroup>
        ...
        <FormControl componentClass="select" name="itemPosition" defaultValue={String(profileStructure.length)}>
          {positionOptions.map((name, index) => (
            <option value={String(index)} key={String(index)}>
              {name}
            </option>
          ))}
        </FormControl>
      ...
      <FormGroup>
        ...
        <FormControl componentClass="select" name="itemType">
          {Constants.profileFieldTypesNames.map((value) => (

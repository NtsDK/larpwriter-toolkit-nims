...
export class RoleGrid {
  ...
  refresh() {
    this.DBMSObj.getRoleGridInfo()
      .then((data2) => {
        ...
        U.showEl(
          U.qe(`${root} > .container-fluid`),
          data2.profileData.length !== 0 && data2.characterProfileStructure.length !== 0
        );
        ...
        const groupingItems = this.profilesData.characterProfileStructure
          .filter(filter)
          .map(R.prop("name"))
          .sort(sorter);

        U.addEls(
          U.clearEl(U.queryEl(`${root}.button-container`)),
          groupingItems.map((item, i) => {
  ...
  onDrop(that) {
    return function (event) {
      ...
      that.updateButtons(JSON.parse(event.dataTransfer.getData("data")), {
        item: this.item,
        order: U.getAttr(this, "order"),
      });
    };
  }
  ...
  getTreeByUserSelect() {
    const groups = R.groupBy(
      (profile) => this.groupingOrder.map((name) => profile.character[name]).join("/"),
      this.profilesData.profileData
    );
  ...
  drawGroupedList(structures) {
    ...
    U.addEl(
      U.clearEl(U.queryEl(`${root}.group-content`)),
      U.addEls(
        U.addClasses(U.makeEl("ul"), ["remove-ul-dots", "zero-padding"]),
  ...
  renderGroupStructure(el) {
    ...
    if (el.children) {
      domChildren = U.addEls(
        U.addClass(U.makeEl("ul"), "remove-ul-dots"),
        R.flatten(el.children.map(this.renderGroupStructure))
      );
  ...
  makeGroupTree(groups, groupingItemInfo, index, key) {
    const arr = groupingItemInfo[this.groupingOrder[index]].value
      .split(",")
      .map((name) => {
  ...
  makePanelList(profileArray) {
    return profileArray.sort(CU.charOrdAFactory((a) => a.characterName.toLowerCase())).map((profileData) => {
      ...
      UI.attachPanelToggler(panelInfo.a, panelInfo.contentDiv, (event, togglePanel) => {
        U.queryEls(`${root}.group-content .expanded[panel-toggler]`)

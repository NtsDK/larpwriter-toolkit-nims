...
function ProfileEditorTmpl(opts) {
  ...
  innerExports.init = () => {
    ...
    const createCharacterDialog = createModalDialog(`.profile-editor2-tab.${`${firstType}-type`}`, createProfile, {
  ...
  function rebuildInterface(primaryNames, secondaryNames, profileBindings) {
    ...
    U.addEls(
      U.clearEl(U.queryEl(`${root} .entity-list`)),
      primaryNames.map((name, i, arr) => {
        const content = U.makeEl("div");
    ...
    DBMS.getProfileStructure({ type: firstType })
      .then((allProfileSettings) => {
  ...
  function showCharacterReports(name) {
    Promise.all([DBMS.getCharacterReport({ characterName: name }), DBMS.getRelationsSummary({ characterName: name })])
      .then((results) => {
        ...
        if (characterReport.length !== 0) {
          ...
          U.addEls(U.clearEl(U.queryEl(reportByStoriesDiv)), characterReport.map(CharacterReports.makeStoryReportRow));
        }
        ...
        if (relationsSummary.relations.length !== 0) {
          ...
          relationsSummary.relations.sort(
            CU.charOrdAFactory((rel) => ProjectUtils.get2ndRelChar(name, rel).toLowerCase())
          );

          U.addEls(
            U.clearEl(U.queryEl(reportByRelationsDiv)),
            relationsSummary.relations.map(CharacterReports.makeRelationReportRow(name))
          );
        }
      })
      .catch(UI.handleError);
  ...
  function filterOptions(event) {
    ...
    if (
      U.queryEl(`${root} .hidden[primary-name] .select-button.btn-primary`) !== null ||
  ...
  function createProfile(dialog) {
    return () => {
      ...
      DBMS.createProfile({ type: firstType, characterName: value })
        .then(() => {
  ...
  function removeProfile(type, name, btn) {
    return () => {
      UI.confirm(CU.strFormat(l10n(opts.removeMsg), [name]), () => {
        DBMS.removeProfile({ type, characterName: name })
          .then(() => {
            PermissionInformer.refresh()
              .then(() => {

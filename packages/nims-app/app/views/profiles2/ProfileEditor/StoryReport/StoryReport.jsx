...
const makeCompletenessLabel = (value, total) =>
  CU.strFormat("{0} ({1}/{2})", [total === 0 ? "-" : `${((value / total) * 100).toFixed(0)}%`, value, total]);
...
export function StoryReport(props) {
  ...
  useEffect(() => {
    dbms
      .getCharacterReport({ characterName: id })
      .then((characterReport) => {
  ...
  return (
    <PanelCore className="story-report" title={t("profiles.character-report-by-stories")}>

...
export class CharacterProfile extends Component {
  ...
  refresh() {
    ...
    Promise.all([dbms.getProfileStructure({ type: "character" }), dbms.getProfile({ type: "character", name: id })])
      .then((results) => {
    ...
    switch (profileItemModel.type) {
      ...
      case "enum":
        ...
        return (
          ...
          >
            {options.map((option) => (
              <option value={option} selected={profile[profileItemModel.name] === option}>
                {option}
              </option>
            ))}
          </FormControl>
  ...
  render() {
    ...
    return (
      <PanelCore className="character-profile" title={t("profiles.character-profile")}>

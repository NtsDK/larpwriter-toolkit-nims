...
function ProfileConfigurerTmpl(opts) {
  ...
  innerExports.init = () => {
    ...
    const createProfileItemDialog = createModalDialog(
      ...
      createProfileItem,
      {
        ...
        initBody: (body) => {
          ...
          const fillMainSel = () => {
            fillItemTypesSel(U.clearEl(sel));
          };
          fillMainSel();
    ...
    state.moveProfileItemDialog = createModalDialog(`.profile-configurer2-tab.${`${tabType}-type`}`, moveProfileItem, {
    ...
    state.renameEnumItemDialog = createModalDialog(`.profile-configurer2-tab.${`${tabType}-type`}`, renameEnumValue, {
    ...
    state.enumEditorDialog = createModalDialog(`.profile-configurer2-tab.${`${tabType}-type`}`, updateEnumValues, {
      ...
      initBody: (body) => {
        ...
        U.listen(inputArea, "input", () => {
          const newVals = inputArea.value
            .split(",")
            .map(R.trim)
            .filter(R.pipe(R.equals(""), R.not));
    ...
    state.multiEnumEditorDialog = createModalDialog(`.profile-configurer2-tab.${`${tabType}-type`}`, updateEnumValues, {
  ...
  function refreshPanel(type, root) {
    Promise.all([DBMS.getProfileStructure({ type }), PermissionInformer.isAdmin()])
      .then((results) => {
        ...
        const arr = allProfileSettings.map(
          R.compose(CU.strFormat(L10n.getValue("common-set-item-before")), R.append(R.__, []), R.prop("name"))
        );
        arr.push(L10n.getValue("common-set-item-as-last"));

        const positionSelectors = [
          U.queryEl(`${tabRoot} .create-entity-position-select`),
          U.queryEl(`${tabRoot} .move-entity-position-select`),
        ];
        positionSelectors
          .map(U.clearEl)
          .map(U.fillSelector(R.__, U.arr2Select(arr)))
          .map(U.setProp(R.__, "selectedIndex", allProfileSettings.length));
  ...
  // eslint-disable-next-line no-var,vars-on-top
  var getInput = R.curry((type, profileSettings, index) => {
      ...
      switch (itemType) {
        ...
        case "enum":
          ...
          if (missedValues.length !== 0) {
            UI.confirm(
              CU.strFormat(L10n.getValue("profiles-new-enum-values-remove-some-old-values"), [missedValues.join(",")]),
              updateEnum,
              () => {
                event.target.value = oldValue;
              }
  ...
  function changeProfileItemType(type) {
    return (event) => {
      UI.confirm(
        CU.strFormat(L10n.getValue("profiles-are-you-sure-about-changing-profile-item-type"), [event.target.info]),
        () => {

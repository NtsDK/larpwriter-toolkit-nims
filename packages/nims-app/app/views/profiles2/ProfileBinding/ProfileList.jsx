      ...
      {(provided) => (
        <div className="entity-list tw-flex tw-flex-col" ref={provided.innerRef} {...provided.droppableProps}>
          {profileNames.map((profile, i) => (
            <Draggable draggableId={JSON.stringify([profileType, profile.displayName])} index={i}>
              {(provided2, snapshot) => (
                <div
                  className={classNames("btn btn-default tw-flex-auto tw-text-left", {
                    "tw-border-dashed tw-border-2 tw-border-blue-900":
                      snapshot.combineTargetFor && JSON.parse(snapshot.combineTargetFor)[0] !== profileType,
                  })}

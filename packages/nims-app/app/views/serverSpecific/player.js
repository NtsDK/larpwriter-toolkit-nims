...
function refresh() {
  Promise.all([DBMS.getWelcomeText(), DBMS.getPlayerProfileInfo(), DBMS.getPlayersOptions()])
    .then((results) => {
...
function buildInterface(text, profileInfo, playersOptions) {
  ...
  U.addEl(
    U.clearEl(U.queryEl(playerHeader)),
    U.makeText(CU.strFormat(L10n.getValue("briefings-player-profile"), [profileInfo.player.profile.name]))
  );

  if (profileInfo.character === undefined) {
    U.addEl(
      U.clearEl(U.queryEl(characterHeader)),
      U.makeText(CU.strFormat(L10n.getValue("briefings-character-profile"), [""]))
    );
    const el = U.clearEl(U.queryEl(characterProfileDiv));
    if (playersOptions.allowCharacterCreation) {
      const label = U.addEl(
        U.makeEl("div"),
        U.makeText(L10n.getValue("profiles-player-has-no-character-and-can-create-it"))
      );
      ...
      U.addEl(
        el,
        U.addEl(U.makeEl("span"), U.makeText(L10n.getValue("profiles-player-has-no-character-and-cant-create-it")))
      );
    }
  } else {
    profileEditorCore.initProfileStructure(characterProfileDiv, "character", profileInfo.character.profileStructure);
    profileEditorCore.fillProfileInformation(
      characterProfileDiv,
      "character",
      profileInfo.character.profile,
      isEditable
    );
    U.addEl(
      U.clearEl(U.queryEl(characterHeader)),
      U.makeText(CU.strFormat(L10n.getValue("briefings-character-profile"), [profileInfo.character.profile.name]))
    );
  }

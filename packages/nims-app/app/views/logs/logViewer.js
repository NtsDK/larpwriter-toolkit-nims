...
export class LogViewer {
  ...
  getData(event) {
    ...
    DBMS.getLog({ pageNumber: Number(event.target.value), filter })
      .then(this.dataRecieved)
  ...
  makeRow(rowData) {
    ...
    return U.addEls(U.makeEl("tr"), [
      ...
      U.addEls(
        U.makeEl("td"),
        JSON.parse(rowData[4]).map((item) =>
          U.addEl(
            U.addClass(U.makeEl("div"), "log-param"),
            U.makeText(R.is(Object, item) ? JSON.stringify(item) : item)
          )
        )
      ),
      U.addEls(
        U.makeEl("td"),
        JSON.parse(rowData[5]).map((item) =>
          U.addEl(
            U.addClass(U.makeEl("div"), "log-param"),
            U.makeText(R.is(Object, item) ? JSON.stringify(item) : item)
          )

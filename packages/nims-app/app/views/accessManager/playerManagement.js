...
function init() {
  ...
  $(`${root}.change-password-user-select`)
    .select2()
    .on("change", (event) => {
...
function createUser(dialog) {
  return () => {
    ...
    DBMS.createPlayer({ userName: userNameInput.value.trim(), password: userPasswordInput.value })
      .then(() => {
...
function createUserAccount(dialog) {
  return () => {
    ...
    DBMS.createPlayerLogin({ userName: userNameSelect.value, password: passwordInput.value })
      .then(() => {
        passwordInput.value = "";
        dialog.hideDlg();
        refresh();
...
function changePassword(dialog) {
  return () => {
    ...
    DBMS.changePlayerPassword({ userName, newPassword })
      .then(() => {
        dialog.hideDlg();
        refresh();
      })

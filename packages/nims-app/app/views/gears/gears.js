...
function init() {
  ...
  U.listen(U.queryEl(`${root} .physics-enabled-checkbox`), "change", (event) => {
    DBMS.setGearsPhysicsEnabled({ enabled: event.target.checked })
      .then(() => {
  ...
  U.listen(U.queryEl(`${root} .show-notes-checkbox`), "change", (event) => {
    DBMS.setGearsShowNotesEnabled({ enabled: event.target.checked }).then(refresh).catch(UI.handleError);
  });
    ...
    manipulation: {
      ...
      addEdge(data, callback) {
        ...
        if (data.from === data.to) {
          UI.confirm(
            l10n("do-you-want-to-connect-node-to-itself"),
            () => {
              callback(data);
            },
            () => callback()
...
function storeData(callback) {
  DBMS.setGearsData({ data: exportNetwork() })
    .then(() => {
...
function updateNodeTextArea() {
  document.querySelector(".nodesText").value = state.nodesDataset
    .map((node) => [node.name, node.group, node.notes].join("\t"))
    .join("\n");
}

function updateEdgeTextArea() {
  document.querySelector(".edgesText").value = state.edgesDataset
    .map((edge) =>
      [state.nodesDataset.get(edge.from).name, edge.label, state.nodesDataset.get(edge.to).name].join("\t")
    )
    .join("\n");
}
...
function downloadCsv() {
  ...
  const arr2 = state.edgesDataset.map((edge) => [
    state.nodesDataset.get(edge.from).name,
    edge.label,
...
function downloadYED() {
  ...
  const nodes = state.nodesDataset
    .map((node) => {
  ...
  const edges = state.edgesDataset
    .map((edge) => CU.strFormat(Constants.yedEdgeTmpl, [edge.id, edge.label || "", edge.from, edge.to]))
    .join("\n");

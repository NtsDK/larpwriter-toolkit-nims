  ...
  if (active) {
    ...
    return (
      <div className="custom-tooltip">
        <p className="intro">{payload[0].payload.tooltipLabel || payload[0].payload.label}</p>
      </div>
    );
  }
        ...
        <RechartsPieChart>
          <Pie dataKey="value" data={data} innerRadius={30} outerRadius={55}>
            {data.map((entry, index) => (
              <Cell fill={Constants.colorPalette[index % Constants.colorPalette.length].color.background} />
            ))}
          </Pie>

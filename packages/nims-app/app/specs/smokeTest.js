        ...
        DBMS[check.func](check.args)
          .then((res) => {
            ...
            if (check.forInconsistency === true) {
              // if (true) {
              DBMS.getConsistencyCheckResult()
                .then((checkResult) => {
...
describe("Core smoke test coverage check", () => {
  ...
  it("Core smoke test coverage check", () => {
    const funcArr = R.uniq(
      R.concat(R.flatten(R.values(getChecks)).map(R.prop("func")), R.flatten(R.values(setChecks)).map(R.prop("func")))
    );

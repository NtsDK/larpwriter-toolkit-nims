...
function StoriesPage() {
  ...
  const handleCreate = async () => {
    if (newName.trim()) {
      await stories.create(newName.trim());
      setNewName('');
      close();
    }
  };
  ...
  const handleRemove = async (name: string) => {
    if (confirm(`Удалить сюжет "${name}"?`)) {
      await stories.remove(name);
    }
  };
  ...
  return (
    <Stack gap="lg">
      ...
      {stories.names.length === 0 && <Text c="dimmed">{t('common.noData')}</Text>}

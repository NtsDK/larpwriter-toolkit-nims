...
export function makeNewBase(base) {
  return new Promise((resolve, reject) => {
    UI.confirm(
      L10n.getValue("utils-new-base-warning"),
      () => {
        DBMS.setDatabase({ database: R.clone(base) })
          .then(() => {
            resolve(true);
...
export function saveFile() {
  DBMS.getDatabase()
    .then((database) => {
      json2File(
        database,
        makeFileName(`${PROJECT_NAME}_${database.Meta.name}`, "json", new Date(database.Meta.saveTime))
      );
    })
